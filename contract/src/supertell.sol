 ...
 * @title SuperTell
 */
contract SuperTell is Ownable, Pausable, ReentrancyGuard {
    ...
    function claim(uint256[] calldata epochs) external nonReentrant notContract {
        ...
        if (reward > 0) {
            _safeTransferGAS(address(msg.sender), reward);
        }
    }
    ...
    function claimTreasury() external nonReentrant onlyAdmin {
        ...
        treasuryAmount = 0;
        _safeTransferGAS(adminAddress, currentTreasuryAmount);
    ...
    /**
     * @notice Transfer GAS in a safe way
     * @param to: address to transfer GAS to
     * @param value: GAS amount to transfer (in wei)
     */
    function _safeTransferGAS(address to, uint256 value) internal {
        (bool success, ) = to.call{value: value}("");
        require(success, "TransferHelper: GAS_TRANSFER_FAILED");
    }
